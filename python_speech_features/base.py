...
def xfc(name, signal, samplerate=16000, winlen=0.025, winstep=0.01, nfilt=26, nfft=512, lowfreq=0,
         highfreq=None, preemph=0.97, winfunc=lambda x: numpy.ones((x,)), log=True, return_energy=False, **kwargs):
    """
    Calculate x-scale frequency coefficients (no cepstral).
    :param return_energy: if True will return the DC as a separate return value
    :param name: name of the 'X' scale, can be 'mfcc', 'gfcc', 'lfcc', 'bfcc'
    ...
    """
    if name == 'mfcc':
        scaling = (hz2mel, mel2hz)
    elif name == 'bfcc':
        scaling = (hz2bark, bark2hz)
    elif name == 'lfcc':
        scaling = (hz2hz, hz2hz)
    elif name == 'gfcc':
        lowhear = kwargs['lowhear']
        hihear = kwargs['hihear']
        k = kwargs['k']
...
def xfcc(name, signal, samplerate=16000, winlen=0.025, winstep=0.01, numcep=13, nfilt=26, nfft=512, lowfreq=0,
    ...
    """
    feat, energy = xfc(name, signal, samplerate, winlen, winstep, nfilt, nfft, lowfreq, highfreq, preemph, winfunc,
                       log=True, return_energy=True, **kwargs)
    xtrargs = {'axis': 1}
    if cepsfunc == 'dct':
        feat = dct(feat, type=2, norm='ortho')[:, :numcep]
        feat = lifter(feat, ceplifter)
    elif cepsfunc == 'dst':
        feat = dst(feat, type=2, norm='ortho')[:, :numcep]
        feat = lifter(feat, ceplifter)
    else:
        feat = fft(feat, **xtrargs)[:, :numcep]
        if cepsfunc == 'psd':
            feat = fft(feat, **xtrargs)[:, :numcep]
            feat = lifter(feat, ceplifter)
            feat = numpy.abs(feat)
            ...
            feat = fft(feat, **xtrargs)[:, :numcep]
            feat = numpy.unwrap(numpy.angle(feat.imag))
